...
@section('page-title', 'Ajouter une intervention')

@section('content')
@php
    $currentUser = auth()->user();
    $isTechnician = in_array((string) ($currentUser?->role ?? ''), ['technicien', 'technician'], true);
    $defaultTechnicianName = old('technician_name', $isTechnician ? (string) ($currentUser?->name ?? $currentUser?->login ?? '') : '');
...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-md p-8">
    ...
    @if (collect($equipments ?? [])->isEmpty())
        <div class="mb-6 rounded-lg border border-yellow-200 bg-yellow-50 px-4 py-3 text-yellow-800">
            Aucun équipement disponible. Ajoutez d'abord un équipement dans le module Équipements.
        </div>
    @endif

    <form class="grid grid-cols-1 md:grid-cols-2 gap-6" method="POST" action="{{ route('interventions.store') }}">
        @csrf
        <input value="Code généré automatiquement par GMAO" class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600" readonly>
        <div class="relative">
            <input type="hidden" name="equipment_id" id="equipment_id_create" value="{{ $selectedEquipmentId > 0 ? $selectedEquipmentId : '' }}" required>
            <input type="text" id="equipment_search_create" class="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="Sélectionner un équipement" autocomplete="off" required>
            <div id="equipment_results_create" class="hidden absolute left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-72 overflow-y-auto z-30"></div>
        </div>
        ...
        <select name="type" class="px-4 py-2 border border-gray-300 rounded-lg" required>
            ...
            <option value="Prédictive" {{ old('type') === 'Prédictive' ? 'selected' : '' }}>Prédictive</option>
            <option value="Améliorative" {{ old('type') === 'Améliorative' ? 'selected' : '' }}>Améliorative</option>
            <option value="Systématique" {{ old('type') === 'Systématique' ? 'selected' : '' }}>Systématique</option>
            <option value="Urgente" {{ old('type') === 'Urgente' ? 'selected' : '' }}>Urgente</option>
        </select>
        <select id="maintenance_scope_create" name="maintenance_scope" class="px-4 py-2 border border-gray-300 rounded-lg" required>
            <option value="interne" {{ old('maintenance_scope', 'interne') === 'interne' ? 'selected' : '' }}>Interne</option>
            <option value="externe" {{ old('maintenance_scope') === 'externe' ? 'selected' : '' }}>Externe</option>
        </select>
        ...
        <input type="date" name="date_start" value="{{ old('date_start') }}" class="px-4 py-2 border border-gray-300 rounded-lg">

        <div id="external_call_box_create" class="md:col-span-2 hidden rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
            ...
            <p class="mt-1 text-xs text-blue-700">Si coché, un ticket SAV externe est ouvert automatiquement à l'enregistrement.</p>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const form = document.querySelector('form[action="{{ route('interventions.store') }}"]');
    ...
    const maintenanceScopeSelect = document.getElementById('maintenance_scope_create');
    const externalCallBox = document.getElementById('external_call_box_create');

    if (!form || !hiddenInput || !searchInput || !resultsBox || !maintenanceScopeSelect || !externalCallBox) {
        return;
    }
