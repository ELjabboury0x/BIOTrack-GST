...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-md p-8">
    <h2 class="text-xl font-bold text-gray-800 mb-6">Formulaire Planning Sociétés Externes</h2>
    ...
    <form class="grid grid-cols-1 md:grid-cols-2 gap-6" method="POST" action="{{ route('planning.store') }}">
        @csrf
        <select name="company_id" class="px-4 py-2 border border-gray-300 rounded-lg" required>
            ...
            @foreach(($companies ?? collect()) as $company)
                <option value="{{ $company->id }}" {{ (int) old('company_id') === (int) $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
            @endforeach
        </select>
        <div class="md:col-span-1">
            <label class="block text-sm font-semibold text-gray-700 mb-2">Date début</label>
            <input type="date" id="planned_date" name="planned_date" value="{{ old('planned_date') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
        </div>
        <div class="md:col-span-1">
            <label class="block text-sm font-semibold text-gray-700 mb-2">Date fin</label>
            <input type="date" id="planned_date_end" name="planned_date_end" value="{{ old('planned_date_end') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
        </div>
        ...
        <input name="contact_person" value="{{ old('contact_person') }}" placeholder="Intervenant" class="px-4 py-2 border border-gray-300 rounded-lg">
